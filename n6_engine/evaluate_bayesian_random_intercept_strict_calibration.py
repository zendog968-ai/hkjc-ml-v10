...
`target_accept=0.995`, after which posterior diagnostics, race normalization and
...
EXPERIMENT_ID = "bayesian_random_intercept_strict_v1"
...
def fit_calibrator(calibration: pd.DataFrame, seed: int) -> tuple[dict[str, np.ndarray | float], pd.DataFrame, dict[str, float]]:
    ...
    with pm.Model(coords={"going": list(GOINGS), "parent": list(PARENTS), "obs": np.arange(len(aggregate))}) as model:
        ...
        sigma_alpha = pm.HalfNormal("sigma_alpha", sigma=0.15, dims="parent")
...
def write_report(summary: dict[str, Any]) -> None:
    lines = [
        ...
        "",
        "> 此候選以 Going 隨機截距向父曲面類型部分池化，並以一個全域正斜率校準 N6 logit。它移除了低樣本 Going 的獨立斜率，以降低識別問題；每折使用 `target_accept=0.995` 的 NUTS。候選不會改動生產模型、API 或 V10 資料。",
        "",
