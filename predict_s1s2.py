...
def main() -> int:
    ...
    try:
        ...
        for runner in runners:
            ...
            enrichment = feature_enrichment(prior_conn, {**race, "meeting_date": race.get("meeting_date")}, runner, len(runners), generated_at, field_rating_mean, drops.get(int(runner["horse_no"])), args.odds_drop_log_weight)
    ...
    for runner, win_p, place_p, prior, enrichment in zip(runners, win, place, meta, enrichments):
        ...
        result = {
            ...
            "odds_snapshot_status": odds_status, "odds_snapshot_at_utc": card.get("odds_snapshot_at_utc"), "odds_drop_flag": enrichment["odds_drop_flag"], "source_json_path": str(input_path.resolve()), "international_rating": enrichment["international_rating"], "rating_type": enrichment["rating_type"], "days_since_last_run": enrichment["days_since_last_run"], "going_suitability": enrichment["going_suitability"], "trainer_g1_win_rate": enrichment["trainer_g1_win_rate"], "odds_drop_ratio": enrichment["odds_drop_ratio"], "odds_drop_weight": enrichment["odds_drop_weight"], "feature_detail": enrichment["detail"],
        }
