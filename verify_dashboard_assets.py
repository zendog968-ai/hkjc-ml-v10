...
def main() -> int:
    ...
    for expected in ("/api/races/", "/api/prediction/", "/api/double-trio/", "/api/double-trio/backtest", "/api/report/", "renderDoubleTrio", "renderDoubleTrioBacktest", "cohorts", "oddsMovementClass", "T-15", "T-5", "DOMPurify.sanitize", "positive-ev", "kelly_quarter_fraction_capped"):
        require(APP, expected, "frontend/app.js")
    for expected in (".positive-ev", ".markdown-body", ".race-button", ".double-trio-event", ".double-trio-capital", ".double-trio-backtest", ".backtest-na", ".backtest-exploratory", ".odds-movement-shortening", ".odds-movement-drift", ".odds-summary-alert"):
    ...
    print(json.dumps({
        "status": "PASS",
        "ui_contract": "date selector + race list + prediction table + official Double Trio strategy with odds movement and cohort-isolated historical backtest view + report view",
