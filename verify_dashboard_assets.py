...
def main() -> int:
    for expected in ("bootstrap@5.3.3", "marked@12.0.2", "dompurify@3.1.5", 'id="raceDate"', 'id="loadRacesButton"', 'id="raceList"', 'id="predictionTableBody"', 'id="reportContent"'):
        require(INDEX, expected, "frontend/index.html")
    for expected in ("/api/races/", "/api/prediction/", "/api/report/", "DOMPurify.sanitize", "positive-ev", "kelly_quarter_fraction_capped"):
        require(APP, expected, "frontend/app.js")
    for expected in (".positive-ev", ".markdown-body", ".race-button"):
    ...
    print(json.dumps({
        "status": "PASS",
        "ui_contract": "date selector + race list + prediction table + report view",
