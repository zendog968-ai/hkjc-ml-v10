...
def main() -> int:
    ...
    for expected in ("/api/races/", "/api/prediction/", "/api/double-trio/", "/api/double-trio/backtest", "/api/report/", "renderDoubleTrio", "renderDoubleTrioBacktest", "renderOverseasDeep", "loadOverseasDeep", "overseasRaceSelector", "loadOverseasDeepButton", "/api/overseas-deep/", "disabled_non_hk", "not_requested", "scrapeStatus", "overseas-partial", "marketReady", "market_research", "overseasEv", "overseas-positive-ev", "racing_post_rating", "top_speed_rating", "cohorts", "oddsMovementClass", "T-15", "T-5", "DOMPurify.sanitize", "positive-ev", "kelly_quarter_fraction_capped"):
