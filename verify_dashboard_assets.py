...
def main() -> int:
    ...
    for expected in ("/api/races/", "/api/prediction/", "/api/double-trio/", "/api/report/", "renderDoubleTrio", "DOMPurify.sanitize", "positive-ev", "kelly_quarter_fraction_capped"):
        require(APP, expected, "frontend/app.js")
    for expected in (".positive-ev", ".markdown-body", ".race-button", ".double-trio-event", ".double-trio-capital"):
    ...
    print(json.dumps({
        "status": "PASS",
        "ui_contract": "date selector + race list + prediction table + official Double Trio strategy view + report view",
