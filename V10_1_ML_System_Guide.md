...
| 月度工作流程 | `monthly_update.py` | 順序執行增量抓取、結果清理、特徵重建及模型重訓。 |
...
資料庫涵蓋 2025/26 馬季 **88 個賽日、868 場賽事紀錄與 10,947 筆馬匹出賽紀錄**。取消、宣布無效及撤回狀態會保留作稽核，但不會進入勝率訓練樣本。V10.1 特徵庫含 **10,381 筆** 無未來資料的可訓練列，覆蓋 1,427 匹馬與 41 名騎師。
...
|---|---:|---|
| 首選（Top 1）勝出率 | 17.4% | 在 138 場測試賽事中，模型排名第 1 的馬有約 1/6 勝出；不代表單場保證。 |
| 首三名（Top 3）包含頭馬比率 | 46.4% | 頭馬有約 46% 機會出現在模型頭三名之內。 |
| 場內正規化平均 Brier score | 0.8922 | 低於等機會基準 0.9192，代表校準較「每匹馬一律同機會」為佳。 |
| Row-level ROC-AUC | 0.6812 | 模型對頭馬與非頭馬有一定排序辨識力，但絕非可保證的預測能力。 |
...
程式會由資料庫最後已記錄賽日的翌日開始抓取，沿用單線程、隨機延遲與定期冷卻。遇到 HTTP 403 或 429 會停止而不規避限制，之後可重跑續接。完成後會自動重建 ELO／跑道／班磅特徵並重訓 `horse_model.pkl`。
