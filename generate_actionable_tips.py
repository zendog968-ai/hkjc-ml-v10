...
def render(payload: dict[str, Any], label: str) -> str:
    ...
    lines = [f"下注實戰精選方案（{label}）", odds_status_label(payload), "獨贏（Win）/ 位置（Place）："]
    if anchor is None:
    ...
    if anchor_ev is not None and anchor_ev < 0:
        ...
        anchor_note += "；EV 未確認"
    lines.append(f"{anchor_no}號「{anchor_name}」（預測勝率 {pct(anchor['_probability'])}%，{anchor_note}）。")
    ...
    lines.append(f"以 {anchor_ref[0]}號「{anchor_ref[1]}」 為單膽，拖 {drag}（{anchor_ref[0]} 拖 {drag_numbers}）。")
    if len(leg_refs) >= 2:
        lines.append("二重彩單式：")
